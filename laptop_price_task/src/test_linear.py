...
def main() -> None:
	...
	model_path = models_dir / "laptop_linear_model.pkl"
	...
	assert feature_names[0] == "__bias__"
	...
	print("Laptop test metrics (linear):")
	print(f"  MSE: {metrics['mse']:.4f}  MAE: {metrics['mae']:.4f}  R2: {metrics['r2']:.4f}")

	pred_df = pd.DataFrame({"y_true": y_test, "y_pred": y_pred})
	pred_path = results_dir / "laptop_linear_test_predictions.csv"
	pred_df.to_csv(pred_path, index=False)

	metrics_path = results_dir / "laptop_linear_test_metrics.txt"
