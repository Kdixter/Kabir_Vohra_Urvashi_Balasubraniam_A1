...
def main():
    ...
    df = engineer_features(data_df)
    df = impute_numeric_median(df)
    
    corr_df = load_target_correlations(results_dir)
    df = drop_low_correlation_features(df, corr_df, 0.03)
    df = create_interaction_features(df, corr_df, 8)
    
    df = normalize_features_to_unit_range(df)
    df = impute_numeric_median(df)

    X, y_true_price = df_to_design_matrix_poly(df, base_features, degree, target_col)

    # Make predictions (in log space)
    y_pred_log = X @ weights
    # Transform predictions back to price space
    y_pred_price = np.exp(y_pred_log)

    # Calculate metrics in the original price space
    metrics = {
        "Mean Squared Error (MSE)": mse(y_true_price, y_pred_price),
        "Root Mean Squared Error (RMSE)": rmse(y_true_price, y_pred_price),
