...
from train_model_poly import PolynomialRegression
...
def save_results_to_file(metrics: dict, y_true: np.ndarray, y_pred: np.ndarray, project_root: str):
    ...
    results_file = os.path.join(results_dir, 'polynomial_model_test_results.txt')
    
    with open(results_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("LAPTOP PRICE PREDICTION - POLYNOMIAL REGRESSION MODEL TEST RESULTS\n")
        f.write("=" * 80 + "\n\n")
        
        f.write("MODEL INFORMATION:\n")
        f.write("-" * 40 + "\n")
        f.write("Model Type: Polynomial Regression (from scratch)\n")
        f.write("Optimization: Gradient Descent\n")
        f.write("Polynomial Degree: 2\n")
        f.write("Features: 79 (original) -> 3240 (polynomial)\n")
...
def main():
    """Main function to test the polynomial regression model."""
    ...
    print("=" * 60)
    print("LAPTOP PRICE PREDICTION - POLYNOMIAL MODEL TESTING")
    print("=" * 60)
