...
def select_top_base_features_by_variance(train_df: pd.DataFrame, top_k: int) -> List[str]:
	...
	return [c for c in variances.index if c in all_features][:top_k]
...
def df_to_design_matrix_poly(df: pd.DataFrame, base_features: List[str], degree: int, log_target: bool = True) -> Tuple[np.ndarray, np.ndarray, List[str]]:
...
def main() -> None:
	...
	base_features = select_top_base_features_by_variance(train_df, TOP_BASE_FEATURES_FOR_POLY)
	...
	Xb_train, y_train, term_names = df_to_design_matrix_poly(inner_train_df, base_features, POLY_DEGREE, log_target=True)
	Xb_val, y_val, _ = df_to_design_matrix_poly(val_df, base_features, POLY_DEGREE, log_target=True) if len(val_df) else (np.empty((0, Xb_train.shape[1])), np.empty((0,)), term_names)
	...
	for lr in LR_CANDIDATES:
		...
		print(f"LR {lr:.5f}: val MSE={val_mse_score:.6f}, epochs={info['epochs_run']}")

	best = min(results, key=lambda r: r["val_mse"]) if results else {"lr": 1e-3}
	best_lr = best["lr"]
	print(f"Selected learning rate: {best_lr}")

	Xb_full, y_full, term_names_full = df_to_design_matrix_poly(train_df, base_features, POLY_DEGREE, log_target=True)
	...
	Xb_test, y_test_log, _ = df_to_design_matrix_poly(test_df, base_features, POLY_DEGREE, log_target=True)
	y_test_pred_log = predict(Xb_test, w_final)

	print("Training metrics (log-space):")
	print(f"  MSE: {train_mse:.6f}  MAE: {train_mae:.6f}  R2: {train_r2:.6f}")
	...
	with open(model_path, "wb") as f:
		pickle.dump(model, f)
	print(f"Saved polynomial (log-target) model to {model_path}")
