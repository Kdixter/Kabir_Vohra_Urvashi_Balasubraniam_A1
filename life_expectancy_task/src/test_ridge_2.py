...
# Add the current directory to Python path to import the RidgeRegression class
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from train_ridge_2 import RidgeRegression
...
def save_results_to_file(metrics: dict, y_true: np.ndarray, y_pred: np.ndarray, project_root: str):
    """
    ...
    results_file = os.path.join(results_dir, 'ridge_model_results.txt')
    
    with open(results_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("LIFE EXPECTANCY PREDICTION - RIDGE REGRESSION MODEL TEST RESULTS\n")
        f.write("=" * 80 + "\n\n")
        
        f.write("MODEL INFORMATION:\n")
        f.write("-" * 40 + "\n")
        f.write("Model Type: Ridge Regression (L2 Regularization)\n")
        f.write("Optimization: Gradient Descent\n")
        f.write("Features: 200 (including 183 country features)\n")
...
def main():
    """Main function to test the Ridge regression model for life expectancy."""
    ...
    print("=" * 60)
    print("LIFE EXPECTANCY PREDICTION - RIDGE REGRESSION MODEL TESTING")
    print("=" * 60)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("RIDGE REGRESSION TESTING COMPLETED SUCCESSFULLY!")
        print("=" * 60)
        
        # Save results to file
        save_results_to_file(metrics, y_test_clean, y_pred, project_root)
