...
# Add the current directory to Python path to import the PolynomialRegression class
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from train_poly_2 import PolynomialRegression
...
def load_model(model_path: str):
    ...
    Returns:
        Loaded PolynomialRegression model
    """
...
def save_results_to_file(metrics: dict, y_true: np.ndarray, y_pred: np.ndarray, 
    ...
    Args:
        ...
        y_true: True target values
        y_pred: Predicted target values
        feature_names: List of polynomial feature names
        project_root: Path to project root directory
    """
    results_dir = os.path.join(project_root, 'results')
    os.makedirs(results_dir, exist_ok=True)
    
    results_file = os.path.join(results_dir, 'polynomial_model_results.txt')
    
    with open(results_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("LIFE EXPECTANCY PREDICTION - POLYNOMIAL REGRESSION MODEL TEST RESULTS\n")
        f.write("=" * 80 + "\n\n")
        
        f.write("MODEL INFORMATION:\n")
        f.write("-" * 40 + "\n")
        f.write("Model Type: Polynomial Regression with Feature Engineering\n")
        f.write("Optimization: Gradient Descent\n")
        f.write("Original Features: 200 (including 183 country features)\n")
        f.write("Enhanced Features: {} (with interactions and polynomials)\n".format(len(feature_names)))
        ...
        f.write(f"\nTotal enhanced features: {len(feature_names)}\n\n")
...
def main():
    """Main function to test the Polynomial regression model for life expectancy."""
    ...
    print("=" * 60)
    print("LIFE EXPECTANCY PREDICTION - POLYNOMIAL REGRESSION MODEL TESTING")
    print("=" * 60)
    ...
    try:
        ...
        print(f"Total enhanced features used: {len(model.get_feature_names())}")
        ...
        print("\n" + "=" * 60)
        print("POLYNOMIAL REGRESSION TESTING COMPLETED SUCCESSFULLY!")
        print("=" * 60)
